...
class ControllerAppartement extends Controller
{
   ...
   if ($request->ajax()) {
          ...
          $familles = ModelAppartement::select(['id','idmaison','numappartement','typeappartement','typelocation','montantloyer','nbremoiscaution','montanttotalcaution','etatappartement','description','datefin'])->whereNull('datefin')->get();
            ...
            return DataTables::of($familles)
             ...
             ->addColumn('denomination', function($row) {
              ...
              ->addColumn('typeappartement', function($row) {

                    $detenteur=ModelTypeappartement::select(['id','libelletypeappartement'])->where('id', $row->typeappartement)->where('etat', 1)->first();

                    return $detenteur ? ucwords(strtolower($detenteur->libelletypeappartement)) : 'N/A';
               ...
               ->addColumn('montantloyer', function($row) {
                   ...
                   $formattedMontant = number_format($row->montantloyer, 0, ',', ' ');
                    ...
                    $btn = '<div class="montantloyerclass">'.$formattedMontant.'</div>';
 ...
 ->addColumn('montanttotalcaution', function($row) {

                   $formattedMontant = number_format($row->montanttotalcaution, 0, ',', ' ');
                ...
                ->addColumn('typelocation', function($row) {

                    $detenteur=ModelTypelocation::select(['id','libelletypelocation'])->where('id', $row->typelocation)->where('etat', 1)->first();

                    return $detenteur ? ucwords(strtolower($detenteur->libelletypelocation)) : 'N/A';
...
if ($row->etatappartement == 0) {

$btn = '<i class="fas fa-circle f-10 m-r-10"  style="color:red;"></i><label style="color:red;">Libre</label>';
                    }
                    else{
$btn = '<i class="fas fa-circle f-10 m-r-10" style="color:green;" ></i><label style="color:green;">Occupé</label>';
                    }
        ...
        if ($row->etatappartement == 0) {
                    ...
                    $btn = ' <a href="javascript:void(0)" class="btn btn-sm btn-light-danger rollbackappartement" data-id="' . $row->id . '" data-name="' . $row->idmaison . '" data-numappart="' . $row->numappartement . '" data-maison="' . $row->idmaison . '" title="Rollback"  >
                    <i class="feather icon-rotate-ccw"></i></a>';

                      $btn .= ' <a href="javascript:void(0)" class="btn btn-sm btn-light-danger deleteappartementdef" data-id="' . $row->id . '" data-name="' . $row->idmaison . '" data-numappart="' . $row->numappartement . '" data-maison="' . $row->idmaison . '" title="Delete"  ><i class="feather icon-trash-2"></i></a>';

}
else{

     $btn = '<a href="javascript:void(0)" class="btn btn-sm btn-light-success me-1 editappartement" data-id="' . $row->id . '" title="Edit"  ><i class="feather icon-edit"></i></a>';


                        $btn .= ' <a href="javascript:void(0)" class="btn btn-sm btn-light-danger deleteappartement" data-id="' . $row->id . '" data-name="' . $row->idmaison . '" data-numappart="' . $row->numappartement . '" data-maison="' . $row->idmaison . '" title="Delete"  ><i class="feather icon-trash-2"></i></a>';
                ...
                ->rawColumns(['action','etat','montantloyer','montanttotalcaution'])
                ->make(true);
        }

        /*
$familles = ModelAppartement::select([
        'model_maison.id',
...
return DataTables::of($familles)
    ...
    ->make(true);
        */
        ...
        return view('Appartement', compact('types','ville','commune','detenteurs'));*/
    }
   ...
   $validator = \Validator::make($request->all(), [
        ...
        'numappart' => 'required|string|max:255',
        'typeappartement' => 'required|string|max:255',
        'typelocation' => 'required|string|max:255',
        'montantlocation' => 'required|string|max:255',
        'nbmremoiscaution' => 'required|string|max:255',
...
$familles = ModelAppartement::select(['id','idmaison','numappartement','typeappartement','typelocation','montantloyer','nbremoiscaution','montanttotalcaution','etatappartement','description','datefin'])->where('idmaison', $request->denomination)->where('numappartement', $request->numappart)->first();;
...
if($familles){
   ...
   return response()->json([
        'success' => true,
        'message' => 'Numero Appartement existe deja !',
    ], 200);
    ...
    ModelAppartement::create([
        'idmaison' => $request->denomination, // Enregistrer en majuscules
        'numappartement' => $request->numappart,
        'typeappartement' => $request->typeappartement,
        'typelocation' => $request->typelocation,
        'montantloyer' => preg_replace('/\s+/', '',$request->montantlocation),
        'nbremoiscaution' => $request->nbmremoiscaution,
        'montanttotalcaution'=> (preg_replace('/\s+/', '',$request->montantlocation) *$request->nbmremoiscaution),
    ...
    public function update(Request $request, $id)
{
   ...
   /*$validator = \Validator::make($request->all(), [
    ...
    $famille = ModelAppartement::find($id);
    if ($famille) {
...
if($modification_type=="correction"){
       ...
       $famille->update([
        ...
        'typeappartement' => $request->typeappartement,
        'typelocation' => $request->typelocation,
        'montantloyer' => $request->montantlocation,
        'nbremoiscaution' => $request->nbmremoiscaution,
        'montanttotalcaution' =>($request->nbmremoiscaution*preg_replace('/\s+/', '',$request->montantlocation)),
    ...
    $famille->update([
        ...
        ModelAppartement::create([
        'idmaison' => $request->denominationhid, // Enregistrer en majuscules
        'numappartement' => $request->numapparthid,
        'typeappartement' => $request->typeappartement,
        'typelocation' => $request->typelocation,
        'montantloyer' => preg_replace('/\s+/', '',$request->montantlocation),
        'nbremoiscaution' => $request->nbmremoiscaution,
        'montanttotalcaution'=> (preg_replace('/\s+/', '',$request->montantlocation) *$request->nbmremoiscaution),
        'description' => $request->description,
        'etatappartement'=>$famille->etatappartement,
        'datedebut' => now(),
        'datefin' => NULL,
...
public function destroy($id)
{
  ...
  // Return success message with HTTP 200 status
    ...
    public function getNmbreDispoappart(){
        $nmbredispoappart = ModelAppartement::where('etatappartement', 0)->get();

        return response()->json($nmbredispoappart);
    }
}
