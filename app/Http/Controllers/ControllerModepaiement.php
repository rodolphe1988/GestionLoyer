...
class ControllerModepaiement extends Controller
{
    //
   ...
   if ($request->ajax()) {
          ...
          //A $familles = ModelModepaiement::select(['id', 'libellemodepaiement','etat'])->where('etat',1)->get();

          $familles = ModelModepaiement::select(['id', 'libellemodepaiement','etat'])->get();
...
//$btn = '<a href="/famille/'.$row->id.'" class="btn btn-sm btn-warning mx-1 editFamille" data-id="' . $row->id . '" title="Edit" style="background-color: #db6574 !important;" ><i class="fas fa-edit" style="color:white;" ></i></a>';
 ...
 // Bouton Rollback uniquement si etat == 0
        if ($row->etat == 0) {
                    ...
                    $btn = ' <a href="javascript:void(0)" class="btn btn-sm btn-light-danger rollbackmodepaiement" data-id="' . $row->id . '" data-name="' . $row->libellemodepaiement . '" title="Rollback"  >
                    <i class="feather icon-rotate-ccw"></i></a>';

                    $btn .= ' <a href="javascript:void(0)" class="btn btn-sm btn-light-danger deletemodepaiementdef" data-id="' . $row->id . '" data-name="' . $row->libellemodepaiement . '" title="Delete"  ><i class="feather icon-trash-2"></i></a>';
}
else{

     $btn = '<a href="javascript:void(0)" class="btn btn-sm btn-light-success me-1 editmodepaiement" data-id="' . $row->id . '" title="Edit"  ><i class="feather icon-edit"></i></a>';


                        $btn .= ' <a href="javascript:void(0)" class="btn btn-sm btn-light-danger deletemodepaiement" data-id="' . $row->id . '" data-name="' . $row->libellemodepaiement . '" title="Delete"  ><i class="feather icon-trash-2"></i></a>';
    ...
    /*if ($request->ajax()) {
        ...
        $familles = ModelModepaiement::select(['id', 'libellemodepaiement'])->get();
 ...
 // Convertir en majuscules avant validation
    $request->merge([
        'libellemodepaiement' => strtoupper($request->libellemodepaiement),
    ]);


$validator = \Validator::make($request->all(), [
        'libellemodepaiement' => 'required|string|max:255|unique:table_modepaiement,libellemodepaiement',
    ]);
...
if ($validator->fails()) {
 ...
 // Enregistrement en BD
    ModelModepaiement::create([
        'libellemodepaiement' => strtoupper($request->libellemodepaiement), // Enregistrer en majuscules
        'etat' => 1,
    ...
    public function update(Request $request, $id)
{
   ...
   $validator = \Validator::make($request->all(), [
    ...
    $famille = ModelModepaiement::find($id);
    if ($famille) {
...
public function destroy($id)
{
  ...
  // Return success message with HTTP 200 status
    ...
    {
        $treeData = ModelModepaiement::all();
