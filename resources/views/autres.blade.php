...
<!-- [ Header ] end -->
            ...
            <div class="row align-items-center">
              <div class="col-md-12">
                <ul class="breadcrumb">
                  ...
                  <li class="breadcrumb-item" aria-current="page">Autres</li>
                </ul>
              </div>
              <div class="col-md-12">
          ...
          <div class="col-lg-12">
            <div class="card shadow-none">
              <div class="card-header">
                <h5 >Ajouter / Mettre à jour le cachet</h5>
  ...
  border-color: #e58a00;margin-right: 22px !important;">
                      ...
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="exampleModalLabel"
                            ><i data-feather="user" class="icon-svg-primary wid-20 me-2"></i><span id="titlemodepaiement">Ajouter / Mettre à jour le cachet</span></h5
                          >
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"> </button>
                        </div>
                      <form id="cachetForm"  enctype="multipart/form-data">
                            @csrf
       ...
       <input class="form-control" id="idlib" type="hidden" aria-describedby="idlib" name="idlib">
            ...
            <label for="chemin_cachet" class="form-label">Téléverser le cachet</label>
            <input
              type="file"
              class="form-control"
              id="chemin_cachet"
                    ...
                    <option>Option 3</option>
                      ...
                      <input type="file" class="form-control" id="inputGroupFile01" />
                          ...
                          <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-light-primary" id="btenreg" >Enregistrer</button>
                          </div>
                        </form>
                      </div>
                    </div>
                  ...
                  <table id="report-table" class="table table-bordered table-striped mb-0">
                    <thead>
                      <tr>
                        ...
                        <th class="border-top-0" style="text-transform: none !important;text-align: center;">Etat </th>
 ...
 // import { DataTable } from '/assets/js/plugins/module.js';
  ...
  $('#report-table').DataTable({
      ...
      processing: true,
        ...
        ajax: '{{ route("cachet.index") }}', // Fetch data via AJAX
    ...
    buttons: [
        ...
        { data: 'cachet', name: 'cachet', orderable: false, searchable: false, className: 'text-center' },
        { data: 'etat_badge', name: 'etat_badge', className: 'text-center', orderable: false, searchable: false },
        { data: 'action', name: 'action', orderable: false, searchable: false, className: 'text-center' }
 // Operations (edit/delete buttons)
        ],
 drawCallback: function(settings) {
        // Récupère la table DataTable
        var api = this.api();

        // Vérifie le nombre total d'enregistrements dans la table (après filtrage)
        var count = api.data().length;

        // Cache ou affiche le bouton en fonction du résultat
        if (count > 0) {
...
alert(response);
  ...
  <script>
     ...
     $('#cachetForm').submit(function(e) {
            e.preventDefault();
            var id = $('#idlib').val();
           /* var url = (id) ? '/updatemodepaiement/' + id : '/modepaiementi/';
            var message = (id) ? 'Mise à jour Effectuée avec Succès !': 'Insertion effectuée avec Succès !';
            var method = (id) ? 'POST' : 'POST';*/
              var url = (id) ? '/updatecachet/' + id : '/insertioncachet';
           
    var formData = new FormData(this); // <-- IMPORTANT

var method ='POST';
            //alert(url+" "+method+" "+formData);
  ...
  headers: {
        ...
        },       processData: false,  // NE PAS transformer en string
        contentType: false,
                success: function(response) {
  ...
  alert(response.message); // Afficher un message de succès
    ...
    $('#cachetForm')[0].reset();
      $('#exampleModal').modal('hide');
                  /* if(url !='/updatemodepaiement/' + id){

                       $('#modepaiementForm')[0].reset(); // Réinitialiser le formulaire
...
alert(errorMessages);
      ...
      $('body').on('click', '.deletemodepaiement', function() {
        var id = $(this).data('id');
        var name = $(this).data('name');
        var url = '/suppmodepaiement/' + id;
      ...
      // alert(name);
       ...
       $('body').on('click', '.rollbackmodepaiement', function() {
        var id = $(this).data('id');
        var name = $(this).data('name');
        var url = '/rollbackmodepaiement/' + id;
      ...
      $('body').on('click', '.editCachet', function () {
        var id = $(this).data('id');
        
 


       $.get('/cachet/' + id, function (data) {
            // Populate the modal form with the fetched data
           // alert(data.libelefamille);
           $('#titlemodepaiement').html('Mettre à jour le cachet'); 
           $("#idlib").val(id);
           // $('#libellemodepaiement').val(data.libellemodepaiement);
           if (data.chemin_cachet) {
            $('#cachet_preview').html(
                `<img src="${data.chemin_cachet}" alt="Cachet actuel" style="width: 100px; height: auto; margin-top:10px;">`
            );
        }
        $("#btenreg").html('Mettre à jour');
    ...
    $('body').on('click', '.deleteCachet', function () {
    var id = $(this).data('id');

    if (confirm('Voulez-vous vraiment supprimer ce cachet ?')) {
        $.ajax({
            url: '/deletecachet/' + id,
            type: 'GET',
            headers: {
...
$('body').on('click', '.rollbackCachet', function () {
    var id = $(this).data('id');

    if (confirm('Voulez-vous changer l’état de ce cachet ?')) {
        $.ajax({
            url: '/rollbackcachet/' + id,
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function (response) {
                if (response.success) {
                    ...
                    alert(response.message || 'Erreur lors du changement d’état.');
                }
            },
            error: function (xhr) {
                console.error(xhr);
      ...
      $("#buttonshowmodal").on('click',function(){
            ...
            $('#titlemodepaiement').html('Ajouter / Mettre à jour le cachet'); 
           $("#idlib").val("");
            $('#libelledetenteurs').val("");

             $('#cachet_preview').html("");

             $("#btenreg").html('Enregistrer');
