            ...
            <div class="row align-items-center">
              <div class="col-md-12">
                <ul class="breadcrumb">
                  ...
                  <li class="breadcrumb-item" aria-current="page">Type Immobilier</li>
                </ul>
              </div>
              <div class="col-md-12">
                <div class="page-header-title">
                  <h2 class="mb-0">Type Immobilier</h2>
                </div>
              </div>
            </div>
          </div>
                      ...
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="exampleModalLabel"
                            ><i data-feather="user" class="icon-svg-primary wid-20 me-2"></i><span id="titleconstruction">Type Immobilier</span></h5
                          >
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"> </button>
                        </div>
                      <form id="constructionForm" method="post">
                            @csrf
                          <div class="modal-body">
                            ...
                            <div class="mb-3">
                              ...
                              <input
                                type="text"
                                class="form-control"
                                id="libelleconstruction"
                                name="libelleconstruction"
 ...
 // import { DataTable } from '/assets/js/plugins/module.js';
  ...
  $('#report-table').DataTable({
      ...
      processing: true,
        ...
        ajax: '{{ route("affichertypeconstruction") }}', // Fetch data via AJAX
    ...
    buttons: [
        {
            ...
            title: 'Liste Type Immobilier'
        }
    ],
        columns: [
            {data: 'id', name: 'id'}, 
            {data: 'libelleconstruction', name: 'libelleconstruction'}, // Famille name
...
alert(response);
  ...
  <script>
     ...
     $('#constructionForm').submit(function(e) {
            e.preventDefault();
            var id = $('#idlib').val();
            var url = (id) ? '/updatetypeconstruction/' + id : '/typeconstructioni/';
         ...
         $.ajax({
                ...
                success: function(response) {
                    ...
                    /*alert(''+message);
                     ...
                     $('#constructionForm')[0].reset(); // Réinitialiser le formulaire
...
if (response.success) {
  ...
  alert(response.message); // Afficher un message de succès
    ...
    $('#report-table').DataTable().ajax.reload();
                   if(url !='/updatetypeconstruction/' + id){
      ...
      $('body').on('click', '.deleteconstruction', function() {
        var id = $(this).data('id');
        var name = $(this).data('name');
        var url = '/supptypeconstruction/' + id;
      ...
      // alert(name);
       ...
       $('body').on('click', '.rollbackconstruction', function() {
        var id = $(this).data('id');
        var name = $(this).data('name');
        var url = '/rollbacktypeconstruction/' + id;
      ...
      $('body').on('click', '.editconstruction', function () {
        var id = $(this).data('id');
        
 


       $.get('/gettypeconstructionpar/' + id, function (data) {
            // Populate the modal form with the fetched data
           // alert(data.libelefamille);
           $('#titleconstruction').html('Modifier Libelle Type Immobilier'); 
           $("#idlib").val(id);
            $('#libelleconstruction').val(data.libelleconstruction);
      ...
      $("#buttonshowmodal").on('click',function(){
            ...
            $('#titleconstruction').html('Type Immobilier'); 
           $("#idlib").val("");
