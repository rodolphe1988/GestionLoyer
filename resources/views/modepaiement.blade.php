            ...
            <div class="row align-items-center">
              <div class="col-md-12">
                <ul class="breadcrumb">
                  ...
                  <li class="breadcrumb-item" aria-current="page">Modepaiement</li>
                </ul>
              </div>
              <div class="col-md-12">
                <div class="page-header-title">
                  <h2 class="mb-0">Mode de Paiement</h2>
                </div>
              </div>
            </div>
          </div>
                      ...
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="exampleModalLabel"
                            ><i data-feather="user" class="icon-svg-primary wid-20 me-2"></i><span id="titlemodepaiement">Mode de Paiement</span></h5
                          >
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"> </button>
                        </div>
                      <form id="modepaiementForm" method="post">
                            @csrf
 ...
 // import { DataTable } from '/assets/js/plugins/module.js';
  ...
  $('#report-table').DataTable({
      ...
      processing: true,
        ...
        ajax: '{{ route("affichermodepaiement") }}', // Fetch data via AJAX
    ...
    buttons: [
        ...
        columns: [
            {data: 'id', name: 'id'}, 
            {data: 'libellemodepaiement', name: 'libellemodepaiement'}, // Famille name
...
alert(response);
  ...
  <script>
     ...
     $('#modepaiementForm').submit(function(e) {
            e.preventDefault();
            var id = $('#idlib').val();
...
if (response.success) {
  ...
  alert(response.message); // Afficher un message de succès
    ...
    $('#report-table').DataTable().ajax.reload();
      ...
      $('body').on('click', '.editmodepaiement', function () {
        var id = $(this).data('id');
        
 


       $.get('/getmodepaiementpar/' + id, function (data) {
            // Populate the modal form with the fetched data
           // alert(data.libelefamille);
           $('#titlemodepaiement').html('Modifier Mode Paiement'); 
           $("#idlib").val(id);
      ...
      $("#buttonshowmodal").on('click',function(){
            ...
            $('#titlemodepaiement').html('Mode Paiement'); 
           $("#idlib").val("");
